...
def synthesize_name_table(font: TTFont) -> TTFont:
    ...
    def original_texts(name_id: int) -> list[str]:
        for rec in nt.names:
            if rec.nameID == name_id and (rec.platformID, rec.platEncID, rec.langID) == (
                3,
                1,
                0x409,
            ):
    ...
    derived_ps = (
        re.sub(r"[^A-Za-z0-9_-]", "", derived_full.replace(" ", "")) or "SyntaxFont-Regular"
    )
    ...
    carried = [(nid, original_notices(nid)) for nid in _CARRIED_NAME_IDS if had[nid]]
    orig_full = original_texts(4) or ([_cff_attr("FullName")] if _cff_attr("FullName") else [])
    ...
    for nid, val in entries:
        ...
        if any(
            rec.nameID == nid and (rec.platformID, rec.platEncID, rec.langID) == (1, 0, 0)
            for rec in nt.names
        ):
            try:
...
def colorable_characters(
    ...
    base = {ch: cmap[ord(ch)] for ch in _ALL if ord(ch) in cmap and _fea_safe(cmap[ord(ch)])}
...
def _cff_adder(font: TTFont):
    ...
    private = top.FDArray[0].Private if is_cff2 else getattr(top, "Private", None)
    ...
    if getattr(charstrings, "charStringsAreIndexed", 0):
        charstrings.charStrings = {
            name: charstrings.charStringsIndex[idx] for name, idx in charstrings.charStrings.items()
        }
...
def duplicate_alternates(
    ...
    if "glyf" in font:
        ...
        raise ValueError("unsupported base font: no glyf, CFF or CFF2 outline table")
...
def _referenced_lookups(subtable) -> set:
    ...
    for attr in ("SubRulSet", "SubRuleSet", "SubClassSet", "ChainSubRuleSet", "ChainSubClassSet"):
        for rule_set in getattr(subtable, attr, None) or []:
            rules = (
                getattr(rule_set, "SubRule", None) or getattr(rule_set, "ChainSubRule", None) or []
            )
            for rule in rules:
...
def color_ligature_glyphs(font: TTFont, char_slot: dict, default_slot: str = "value") -> None:
...
def _register_feature(script, feature_index: int) -> None:
    ...
    for langsys in [script.DefaultLangSys] + [r.LangSys for r in (script.LangSysRecord or [])]:
        if langsys is None:
            continue
...
def merge_gsub(base_gsub, our_gsub, region_ids: set[int] | None = None) -> None:
    ...
    other_lookups = [lk for i, lk in enumerate(ours.LookupList.Lookup) if i not in region_ids]
    ...
    other_old = [i for i in range(ours.LookupList.LookupCount) if i not in region_ids]
    mapping = {old: region_pos[old] for old in region_ids}
    ...
    for fr in ours.FeatureList.FeatureRecord:
        ...
        matching = [b for b in base.FeatureList.FeatureRecord if b.FeatureTag == fr.FeatureTag]
        if matching:
            for b in matching:
...
def build_highlight_font(
    ...
    if isolated_languages:
        ...
        fea, _, region_ids = generate_isolated_features(languages, language_ids, glyphs, base)
    else:
    ...
    if base_gsub is not None:
        ...
        char_slot = {
            ch: slot for lang in languages for slot, chars in lang.symbols.items() for ch in chars
        }
