...
Optional per-language keys: `numbers:`, `case_insensitive:` (match words in
upper+lower case), `feature:` (isolated-build tag), `symbols:` (override the
always-colored punctuation), plus:

* `escapes:` — characters that form an escape sequence after `\`, colored with
  the `escape` slot (escaped quotes also do not end a string);
* `formats:` — printf-style specifier characters (`%s`, `%02d`, …) after `%`,
  colored with the `format` slot (defaults to the `escape` color);
* `interpolation:` — string-interpolation delimiters, e.g.
  `{open: '${', close: '}'}` (the expression is highlighted as code).

### 4. Isolated language highlighting
...
In the web app, checking the isolated box makes choosing a sample also switch the
preview to that sample's feature.
...
* Matching is literal and bounded: there are no regular expressions, and a hard
  newline ends every comment/string region.
...
* **Ligatures are dropped by default**; `--keep-ligatures` keeps the base font's
  ...
  Comment and string delimiters still win over ligatures.
